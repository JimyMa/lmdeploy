...
from dlslime import (
    Assignment as DLSlimeAssignment,
    available_nic,
    NVLinkEndpoint,
    RDMAEndpoint,
)
...
class DLSlimeMigrationManagement:
    ...
    def connect(self, kvtransfer_endpoint_info: DistServeKVTransferEndpointInfo):
        self.endpoint[kvtransfer_endpoint_info.protocol].connect(
            json.loads(kvtransfer_endpoint_info.endpoint_info)
        )
...
class DLSlimeBackend(MigrationBackendImpl):
    ...
    def p2p_initialize(self, init_request: DistServeInitRequest):
        self.links[init_request.remote_engine_id] = DLSlimeMigrationManagement(
            init_request
        )
    ...
    def p2p_connect(
        self, remote_engine_id: str, conn_req: DistServeKVTransferEndpointInfo
    ):
        self.links[remote_engine_id].connect(conn_req)

    async def p2p_migrate(
